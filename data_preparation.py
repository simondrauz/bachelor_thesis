...
def preprocess_data(df: pd.DataFrame, covariates: list, lagged_covariates: list, standardize: bool) -> pd.DataFrame:
    """
    ...
    if generate_lags:
        for lag_covariate in lagged_covariates:
            df_lag = generate_lags(df, lag_covariate, list(range(3, 15)))
            # Merge lagged features into the final DataFrame
            data = pd.merge(data, df_lag, on=['month_id', 'country_id'], how='left')

    # Drop rows with missing values which are generated by the lagged features
    data = data.dropna()
