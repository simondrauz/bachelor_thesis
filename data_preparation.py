...
# ToDo: For final use, check that NaN values of data set were handled before dropping NaN from lagged features
def preprocess_data(df: pd.DataFrame, covariates: list, lagged_covariates: list, standardize: bool, lags_needed,
                    drop_na) -> pd.DataFrame:
    """
    Preprocess the data by generating lagged features and standardizing the data.
    Args:
        drop_na:
        lags_needed:
        df: all available data
        covariates: covariates for which we do not generate lagged features
        lagged_covariates: covariates for which we generate lagged features
        standardize: indicator variable whether we want to standardize the data or not

    Returns:

    """
    # Sort the data by 'country_id' and 'month_id'
    df = df.sort_values(by=['country_id', 'month_id'])

    # Select columns to keep in the final DataFrame
    data = df[['month_id', 'country_id', 'ged_sb'] + covariates]

    # Generate lagged features if specified as 3-month till 14-month lags
    if generate_lags:
        for lag_covariate in lagged_covariates:
            df_lag = generate_lags(df, lag_covariate, list(range(3, 14+lags_needed)))
            # Merge lagged features into the final DataFrame
            data = pd.merge(data, df_lag, on=['month_id', 'country_id'], how='left')

    if drop_na:
        # Drop rows with NA values in the target variable
        # Rows where 'country_id' is less than the threshold and any column has NA
        last_month = data['month_id'].max()
        first_allowed_na = last_month - 10
        # Define a mask for rows where 'month_id' is less than the threshold
        mask_lower_month_id = data['month_id'] < first_allowed_na
        # Define a mask for rows where any NA values are present
        mask_na = data.isna().any(axis=1)
        # Combine the masks to identify rows to drop
        rows_to_drop = mask_lower_month_id & mask_na

        # Drop selected rows and return the result
        data = data.drop(data[rows_to_drop].index)

    # Standardize all features if specified except of the target variable, month_id and country_id
    if standardize:
        data = standardization(data)

    # Add columns for each country
    data = add_country_columns(data)
    # Add columns for each season
    data = add_season_columns(data)

    return data


def generate_lags(df: pd.DataFrame, covariate: str, lags: list) -> pd.DataFrame:
    """
    Generate lagged features for a specified covariate.

    Args:
    - df (pd.DataFrame): DataFrame with columns 'month_id', 'country_id', and the covariate of interest.
    - covariate (str): Name of the covariate of interest.
    - lags (list): List of lagged values to generate.

    Returns:
    - df (pd.DataFrame): DataFrame with lagged features for the specified covariate.
    """

    # Sort the data by 'country_id' and 'month_id'
    df = df.sort_values(by=['country_id', 'month_id'])

    # Create a list to store the columns that should be returned
    return_columns = ['month_id', 'country_id', covariate]
    # Initialize empty lists to store new column names
    lagged_cols = []

    # Generate lagged features for the specified covariate
    for lag in lags:
        lagged_col = f'{covariate}_tlag_{lag}'

        # Create lagged features
        df[lagged_col] = df.groupby('country_id')[covariate].shift(lag)

        # Append new column name to list
        lagged_cols.append(lagged_col)

    # Select columns to keep in the final DataFrame
    cols_to_keep = ['month_id', 'country_id'] + lagged_cols
    df = df[cols_to_keep]
...
def standardization(df: pd.DataFrame) -> pd.DataFrame:
    """
     # Standardize all features except of the target variable, month_id and country_id

    Args:
    - df (pd.DataFrame): DataFrame with features.
    - covariates (list): List of covariates to standardize.

    Returns:
    - df (pd.DataFrame): DataFrame with standardized features.
    """
    # Identify columns to standardize
    cols_to_standardize = [col for col in df.columns if col not in ['month_id', 'country_id', 'ged_sb']]
...
def add_country_columns(df):
    """
    Add columns for each country to indicate whether the observation is in the country corresponding to the column.
    Args:
        df: data frame with conflict fatalities and selected covariates

    Returns: data frame with conflict fatalities, selected covariates and country columns

    """
    # Get the unique country IDs
    unique_country_ids = df['country_id'].unique()

    # Create new columns for each country
    for country_id in unique_country_ids:
        # Generate new column name
        new_column_name = f'country_{country_id}'

        # Assign 1 if the observation is in the country corresponding to the column, 0 otherwise
...
def add_season_columns(df):
    """
    Add columns for each season to indicate whether the observation is in the season corresponding to the column.
    Args:
        df: data frame with conflict fatalities and selected covariates (and country columns)

    Returns: data frame with conflict fatalities, selected covariates (and country columns) and season columns

    """
    # Calculate the month based on the given identifier starting from 121
    df['month'] = (df['month_id'] - 121) % 12 + 1

    # Initialize new columns for the seasons
    df['Winter'] = 0
    df['Spring'] = 0
    df['Summer'] = 0
    df['Fall'] = 0
    ...
    df.loc[df['month'].isin([6, 7, 8]), 'Summer'] = 1
    df.loc[df['month'].isin([9, 10, 11]), 'Fall'] = 1

    # Optionally, drop the temporary 'month' column
